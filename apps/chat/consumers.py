...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self) -> None:
        ...
        if not study_group or not await self._is_member(user, study_group):
    ...
    async def chat_message(self, event: Dict[str, Any]) -> None:
        await self.send(text_data=json.dumps({"type": "chat.message", "data": event["data"]}))

    async def user_event(self, event: Dict[str, Any]) -> None:
        """'user.event' 타입의 이벤트를 클라이언트에게 전송"""
        await self.send(text_data=json.dumps({"type": "user.event", "data": event["data"]}))
