...
def recover_account(email: str, verification_code: str) -> None:
    ...
    """
    # 이메일 검증된 데이터인지 확인
    if not email_verified.is_verified(
        email=email, verification_code=verification_code, purpose=VerificationPurpose.RECOVER_ACCOUNT
    ):
