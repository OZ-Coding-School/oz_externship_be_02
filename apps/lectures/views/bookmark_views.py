...
class BookmarkListView(ListAPIView):
    ...
    permission_classes = [permissions.IsAuthenticated]
    serializer_class = BookmarkListItemSerializer
    ...
    @extend_schema(
        ...
        description="로그인 사용자의 북마크 강의 목록을 커서 기반으로 페이지네이션하여 반환합니다.",
        parameters=[
    ...
    def get_queryset(self) -> QuerySet[LectureBookmark]:
        ...
        qs = LectureBookmark.objects.filter(user=user).select_related("lecture")
    ...
    def list(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        ...
        if page is not None:
            lectures: List[Lecture] = [lb.lecture for lb in page]
            serializer = self.get_serializer(lectures, many=True)
            return self.get_paginated_response(serializer.data)

        lectures = [lb.lecture for lb in queryset]
        serializer = self.get_serializer(lectures, many=True)
